...
describe("LendHub Tests", async () => {
  ...
  before(async () => {
    ...
    const abi =
      "../artifacts/contracts/AddressToTokenMap.sol/AddressToTokenMap.json"; // ABI of the smart contract
    const addressToTokenMap = new ethers.Contract(
      AddressToTokenMapAddress,
      abi,
      provider
    );
    ...
    const sym = await addressToTokenMap
      .connect(deployerAddress)
      .getAddress(ETH_ADDRESS);

    console.log("sym : " + sym);

    // const AddressToTokenMapContract = await ethers.getContractAt(
    //   "AddressToTokenMap",
    //   AddressToTokenMapAddress,
    //   deployerAddress
    // );
